  ...
  if (req.method === "GET") {
    try {
      ...
      const employeeMap = employees.reduce((acc, employee) => {
        acc[employee.officeId] = {
          name: employee.name,
          shiftStarts: employee.shiftStarts,
          shiftEnds: employee.shiftEnds,
        };
        return acc;
      }, {});
      ...
      const formattedData = Object.entries(groupedData).map(([employeeId, attendanceRecords]) => ({
        ...
        shiftStarts: employeeMap[employeeId]?.shiftStarts || "",
        shiftEnds: employeeMap[employeeId]?.shiftEnds || "",
        attendanceRecords,
      }));
